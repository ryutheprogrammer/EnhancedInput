...
public:
	...
	const char *getCategoryName() const noexcept
	{
		if (isMouseButton() || isMouseAxis()) return "Mouse";
		if (isKeyboardKey()) return "Keyboard";
		if (isGamepad()) return "Gamepad";
		return nullptr;
	}
